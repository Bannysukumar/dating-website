...
import { Mic, MicOff, Video, VideoOff, PhoneOff, Send, MessageSquare, AlertOctagon, UserX, UserSearch, User as UserIcon } from "lucide-react";
...
export default function Chat() {
  ...
  const { user, currentMatch, setMatch, setIsQueueing } = useAppStore();
  ...
  const { localVideoRef, remoteVideoRef, isMuted, isVideoOff, toggleMute, toggleVideo, callError } = useWebRTC();
  ...
  const handleSkip = () => {
    if (currentMatch) {
      socket.emit("skip_user", { roomId: currentMatch.roomId });
    }
    setMatch(null);
    setIsQueueing(true);
  ...
  const handleEnd = () => {
    if (currentMatch && !partnerDisconnected) {
      socket.emit("disconnect_partner", { roomId: currentMatch.roomId });
    }
    setMatch(null);
    setLocation("/");
  };
  ...
  const handleReport = (reason: string) => {
    if (currentMatch) {
      socket.emit("report_user", { roomId: currentMatch.roomId, reason });
    }
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-background overflow-hidden">
      ...
      <div className="relative w-full lg:w-3/5 h-[40vh] lg:h-full bg-black flex flex-col border-b lg:border-b-0 lg:border-r border-white/10">
        ...
        {/* Overlay if partner disconnected */}
        {partnerDisconnected && (
        ...
        {/* Local Video (PiP) */}
        <div className="absolute bottom-4 right-4 w-24 h-32 sm:w-32 sm:h-48 bg-gray-900 rounded-xl overflow-hidden border-2 border-white/20 shadow-2xl z-10 transition-all">
          <video 
            ref={localVideoRef}
            autoPlay 
            playsInline 
            muted 
            className={`w-full h-full object-cover transition-opacity duration-300 ${isVideoOff ? 'opacity-0' : 'opacity-100'} -scale-x-100`}
          />
          {isVideoOff && (
        ...
        <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/80 to-transparent flex justify-between items-center z-10">
          <div className="glass-card px-4 py-2 rounded-full flex items-center gap-2">
            ...
            <span className="text-xs text-white/50 bg-white/10 px-2 py-0.5 rounded-md ml-1">{currentMatch.partner.gender}</span>
          </div>
          
          <Button variant="ghost" size="icon" onClick={() => setShowReportDialog(true)} className="text-white/50 hover:text-red-400 bg-black/20 backdrop-blur-md rounded-full">
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 z-10 glass-card p-2 rounded-full">
          <Button 
            ...
            size="iconLg" 
            onClick={toggleMute}
            className={`rounded-full transition-colors ${isMuted ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'hover:bg-white/10 text-white'}`}
          >
          ...
          <Button 
            ...
            size="iconLg" 
            onClick={toggleVideo}
            className={`rounded-full transition-colors ${isVideoOff ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'hover:bg-white/10 text-white'}`}
          >
      ...
      <div className="w-full lg:w-2/5 h-[60vh] lg:h-full flex flex-col bg-card/40 backdrop-blur-3xl relative">
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between shrink-0 bg-white/5">
          ...
          <Button variant="outline" size="sm" onClick={handleSkip} className="rounded-full text-xs h-8 px-4 border-primary/50 hover:bg-primary/20 hover:text-white text-white/80">
            Skip & Next
          </Button>
        </div>
        ...
        {showReportDialog && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <p className="text-white/60 text-sm mb-6">Why are you reporting this user? They will be immediately disconnected.</p>
              
              <div className="space-y-3">
                {["Inappropriate behavior", "Harassment or bullying", "Spam", "Other"].map(reason => (
                  <button 
                    key={reason}
